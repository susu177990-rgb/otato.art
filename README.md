...
- **系统 API**：模型和网关由站点管理员统一配置，普通用户无需填写第三方 API Key。
...
- **密钥与数据**：不要提交真实 `.env.local`、Supabase service role key、模型 API Key 或用户数据。生产环境应通过部署平台环境变量管理密钥。
