...
- Primary product surfaces: home (`/`), login (`/login`), unified project settings (`/settings`), standalone chat (`/chat`), Wattpad tools (`/wattpad`), project list (`/projects`), project intake (`/project/[id]/onboarding`), script studio (`/studio/[id]`), image workspace (`/image`, `/image/gallery`), AI+实拍首帧工作台 (`/ai-live-action`).
...
- Core routes/screens: `/`, `/login`, `/settings`, `/chat`, `/image`, `/image/gallery`, `/ai-live-action`, `/projects`, `/project/new`, `/project/[id]/onboarding`, `/studio/[id]`, `/wattpad`.
