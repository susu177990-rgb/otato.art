...
- Primary product surfaces: home (`/`), login (`/login`), personal account center (`/me`), unified project settings (`/settings`), standalone chat (`/chat`), Wattpad tools (`/wattpad`), project list (`/projects`), project intake (`/project/[id]/onboarding`), script studio (`/studio/[id]`), image workspace (`/image`, `/image/gallery`), infinite canvas (`/canvas`, `/canvas/[id]`), AI+实拍首帧工作台 (`/ai-live-action`).
- Evidence reviewed: every page above + `app/shared/shell.module.css` (new shared shell), `app/me/page.tsx`, `app/me/me-page.module.css`, `app/image/image-page.module.css` (canvas + floating panels reference), `app/ai-live-action/page.tsx`, `app/ai-live-action/ai-live-action-page.module.css`, `app/settings/settings-page.module.css`, `app/image/gallery/gallery-page.module.css`, `app/projects/projects-page.module.css`, `app/wattpad/wattpad-page.module.css`, `app/project/[id]/onboarding/onboarding-page.module.css`, `app/studio/[id]/studio-page.module.css`, `components/ApiSettingsProvider.tsx`, `components/SettingsDialog.tsx` (storage helpers only), `components/ChatWindow.tsx`, `components/chat/ChatWorkspace.tsx`, `components/skill-form/*` (RJSF dynamic form + output), `components/ArtifactPanel.tsx`, `components/StudioStageStrip.tsx`, `components/StudioBibleDrawer.tsx`, `components/PlanningChatPanel.tsx`, `components/MessageBubble.tsx`, `components/StageGroup.tsx`, `lib/image-workspace.ts`, `lib/image-storage.ts`, `lib/types.ts`, `lib/model-presets.ts`.
...
- **`/me`**: topbar `返回首页` + active `我的` pill only. Body uses the shared shell and is split into a left "account + security" column and a right "data overview + session notes + boundaries" column. It should feel like a compact account dashboard, not a generic settings form dump. `设置` cannot reappear here because `/settings` remains home-only entry.
- **Unified shell**: every route renders the same chrome — `shellStyles.page` (full-viewport flex column with the radial-haloed near-black gradient background) + `shellStyles.topbar` (unframed top action row with no full-width surface). The topbar's left cluster uses `topbarLeft` and may carry `plainDockText` "back" links (`返回首页` / `返回作图` / `返回项目页` / `返回项目列表`) plus a `topbarTagline` subtitle. The right cluster uses `topnav` and may carry floating `navLink` pills (and optional `navLinkPrimary` / `navLinkDanger` modifiers).
- **`/`** (home): topbar tagline `oTATo Art 工作台` + single `设置` pill. Body is a centered `heroWrap` with a `tileGrid` of large `tile` cards: 扒网文 → `/wattpad`, 创作剧本 → `/projects`, 作图 → `/image`, 无限画布 → `/canvas`, AI+实拍 → `/ai-live-action`, 对话 → `/chat`.
...
- **`/settings`**: current UI-structure pilot for the next page migration. It reads/writes shared `site_settings`, `site_skill_packs`, and the first-stage **预设库** table `site_prompt_presets`, while keeping the project dark glass visual language. Header is an unframed floating action row: `返回首页` + three top-level categories **API设置 / 预设库 / skill** on the left, `保存` + micro-toast on the right, with only the buttons carrying chrome. Body is a two-zone layout: left vertical child nav and right floating-card content. **API设置** child pages: LLM / 图片 / 视频, mapping to LLM API, 生图模型 API, 生视频模型 API. **预设库** child pages: 生图提示词预设 / 生视频提示词预设. **skill** contains Skill 包设置 only. First-stage preset library is deliberately small: each preset has type, title, prompt template, optional cover, and image ref-slot hints; no search, tags, sorting, favorites, versioning, import/export, or batch tools yet.
- Core routes/screens: `/`, `/login`, `/me`, `/settings`, `/chat`, `/image`, `/image/gallery`, `/canvas`, `/canvas/[id]`, `/ai-live-action`, `/projects`, `/project/new`, `/project/[id]/onboarding`, `/studio/[id]`, `/wattpad`.
...
- Spacing/layout rhythm: 52px unframed top action row, 24px page horizontal padding, 12–16px gap between sibling cards, 6–10px field internal gaps. Avoid >24px between related cards.
...
- **`topbar`**, **`topbarLeft`**, **`topbarTagline`**, **`topnav`**: fixed-height unframed top action row; only child buttons/pills draw glass chrome.
...
- Empty: `shellStyles.empty` for catastrophic empty (no project) or post-create flush states. List pages use a small centered helper text. `/me` loading should also stay quiet and centered instead of using a large hero placeholder.
