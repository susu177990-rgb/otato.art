...
- **持久化**：PostgreSQL（`site_settings`、`site_prompt_presets`、`projects`、`image_gallery_records`）；项目和图库按 `auth.users` 隔离，RLS 启用。
...
- **设置页**：LLM / 生图 / 生视频 API Key、预设库与 Skill 写入 Supabase 的全站配置，所有登录账号共用并可修改（需登录）。
