...
import { isKnownImageModeId } from "@/lib/image-workspace";
...
export async function POST(req: Request) {
  try {
    ...
    if (!isKnownImageModeId(modeId, snapshot.imageWorkspace.customModes ?? [])) {
    ...
    const previousUrl = snapshot.imageWorkspace.coverImageUrlByMode?.[modeId];
    const coverImageUrl = await uploadModeCoverObject(auth.supabase, modeId, webpBytes, previousUrl);

    const coverImageUrlByMode = { ...snapshot.imageWorkspace.coverImageUrlByMode, [modeId]: coverImageUrl };
    const nextSnapshot = await upsertWorkspaceSnapshot(auth.supabase, {
      imageWorkspace: { ...snapshot.imageWorkspace, coverImageUrlByMode },
    });

    return NextResponse.json({
      modeId,
      coverImageUrl,
      imageWorkspace: nextSnapshot.imageWorkspace,
    });
  } catch (e) {
    const message = e instanceof Error ? e.message : "upload_failed";
    console.error("[image-mode-covers POST]", e);
...
export async function DELETE(req: Request) {
  try {
    ...
    const coverImageUrlByMode = { ...snapshot.imageWorkspace.coverImageUrlByMode };
    ...
    return NextResponse.json({ modeId, imageWorkspace: nextSnapshot.imageWorkspace });
  } catch (e) {
    const message = e instanceof Error ? e.message : "delete_failed";
    console.error("[image-mode-covers DELETE]", e);
    return NextResponse.json({ error: message }, { status: 400 });
  }
}
