...
import { MODE_COVER_INPUT_MAX_BYTES } from "@/lib/image/process-mode-cover";
import {
  ...
  try {
    ...
    if (file.size > MODE_COVER_INPUT_MAX_BYTES) {
      return NextResponse.json(
        { error: `GIF 封面不能超过 ${Math.round(MODE_COVER_INPUT_MAX_BYTES / (1024 * 1024))}MB` },
        { status: 400 },
      );
    }
    ...
    if (!rawBytes.byteLength) {
      return NextResponse.json({ error: "GIF 封面为空" }, { status: 400 });
    }
    const contentType = (file.type || "").toLowerCase().split(";")[0]?.trim() || "";
    if ((contentType && contentType !== "image/gif") || !isGifBytes(rawBytes)) {
      return NextResponse.json({ error: "生视频预设封面仅支持 GIF 动图" }, { status: 400 });
    }
    const previousUrl = snapshot.videoWorkspace.coverImageUrlByMode?.[modeId];
    const coverImageUrl = await uploadModeCoverObject(auth.supabase, `video-${modeId}`, rawBytes, previousUrl, {
