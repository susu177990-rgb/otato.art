-- Keep persisted reference-image URLs in gallery JSON while still stripping inline payloads.
-- Older compact_image_gallery_records removed the entire referenceImages field.
    ...
    case
      when jsonb_typeof(data->'referenceImages') = 'array' then
        ...
        case
          when coalesce(data->>'imageUrl', '') like 'data:%'
            or length(coalesce(data->>'imageUrl', '')) > 8192
          then data - 'imageUrl' - 'referenceImages'
