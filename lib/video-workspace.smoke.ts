...
assert.equal(getVideoCapabilities("kling-3.0").supportedModes.includes("motion_control"), true);
...
assert.equal(getVideoCapabilities("kling-3.0").supportsMotionControl, true);
...
const klingMotionParams = getVideoParameterCapabilities("kling-3.0", "motion_control");
...
assert.deepEqual(videoModelsForUiMode("motion_control"), ["kling-3.0", "kling-2.6-motion"]);
